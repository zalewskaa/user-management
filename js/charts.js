...
export function createLanguageChart(containerId, data) {
    ...
    data.forEach((user) => {
        languageCounts[user.language] =
            (languageCounts[user.language] || 0) + 1;
    });
...
export function createVersionChart(containerId, data) {
    ...
    data.forEach((user) => {
        if (user.version >= 1.0 && user.version <= 2.0)
            versionRanges["1.0-2.0"]++;
...
// Unused chart functions
export function createUnusedChart1() {
...
export function createUnusedChart2() {
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
...
export function createUnusedChart3() {
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
