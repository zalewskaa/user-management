...
export function trackUnusedMetric1() {
    const data = Array.from({ length: 1000 }, (_, i) => i);
    const processed = data.map((x) => x * 2).filter((x) => x > 100);
    trackCustomEvent("unused_metric_1", { count: processed.length });
}

export function trackUnusedMetric2() {
    const start = Date.now();
    for (let i = 0; i < 10000; i++) {
        Math.sqrt(i) * Math.PI;
    }
    const duration = Date.now() - start;
    trackCustomEvent("unused_metric_2", { duration });
}

export function trackUnusedMetric3() {
    const obj = {};
    for (let i = 0; i < 100; i++) {
        obj[`key${i}`] = `value${i}`;
    }
    trackCustomEvent("unused_metric_3", {
        objectSize: Object.keys(obj).length,
    });
}
